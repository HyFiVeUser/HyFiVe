...
static bool csvLineToJson(const String &line, String &out)
{
  ...
  // The app expects the internal id in the paramUuid field
  // to look up and replace it with the correct uuid on upload
  String paramUuid = col[baseIndex + 0];
  paramUuid.trim();
  ...
  // Use the paramUuid field instead of id.
  // Empty fields break the json parser in the app.
  // We can just skip the lat, lng and svalue fields.
  out = "{";
  out += "\"timestamp\":\"" + ts + "\",";
  out += "\"paramUuid\":\"" + paramUuid + "\",";
  out += "\"nvalue\":" + nval;
  out += "}";
  return true;
}
  ...
  if (layoutReady)
  {
    ...
    else
    {
      ...
      for (int i = 0; i < linesToRead; ++i)
      {
        ...
        String jsonLine;
        if (!csvLineToJson(line, jsonLine))
          continue;
        if (!first)
          json += ",";
        first = false;
