...
void performPeriodicConfigUpdate()
{
  ...
  if (!hasTransmissionUpdateError)
  {
    ...
    if (checkLatestConfigFileExists() && !errorInloggerIdOrTimestamp())
    {
      ...
      {
        Log(LogCategoryConfiguration, LogLevelDEBUG, "update successfull: ", String(findLatestConfigFileUpdateConfig));
  ...
  if (pin20Status_ == HIGH) // Power supply unit not connected when HIGH
  {
    if (getRemainingBatteryPercentage() == 0 || getTotalBatteryCellVoltage() < 11200)
    {
      if (!batteryEmpty)
      {
        ...
        Log(LogCategoryPowerManagement, LogLevelERROR, "Battery empty");
...
void monitorReedInputTask(void *parameter)
{
  ...
  while (true)
  {
    ...
    // Übergang HIGH -> LOW
    if (reedState == LOW && lastState != LOW)
    {
      lowStartMs = now;
      Serial.printf("Pin %u: LOW started\n", REED_INPUT_PIN);
    }

    // Solange LOW: Dauer ausgeben
    if (reedState == LOW)
    {
    ...
    else
    {
      ...
      {
        // Serial.printf("Pin %u state: HIGH\n", REED_INPUT_PIN);
      }
    }
